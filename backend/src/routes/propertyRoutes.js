...
router.delete('/:id', verifyToken, (req, res, next) => {
	...
	Property.findByPk(propertyId).then(property => {
		if (!property) return res.status(404).json({ error: 'Propiedad no encontrada' });
		if (property.sellerId !== req.user.id) {
			return res.status(403).json({ error: 'Acceso denegado: solo el vendedor dueño o admin pueden eliminar esta propiedad' });
		}
		next();
	}).catch(() => res.status(500).json({ error: 'Error de autorización' }));
}, generalLimiter, propertyController.deleteProperty);
